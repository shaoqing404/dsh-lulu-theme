...
- **全组件换装**：覆盖 DSH 全部 107 个 `--dsw-*` 令牌——按钮五大家族、输入框/编写器、消息气泡、侧栏选中条、菜单、滚动条、阴影，以及 SF Pro Rounded 圆体字；不是简单换底色。
- **宠物内置**：水豚噜噜（头顶橘子、手持小风扇）以动画形式常驻界面角落，会摇摇、会挥手、可拖动。
- **一键分发**：单文件插件源码（帧已内嵌），任何 DSH 会话都可以直接安装。
